...
async function loadKlines(){
  try{var r=await fetch('/api/klines/'+state.symbol+'?interval='+state.timeframe+'&limit=90');var j=await r.json();if(!j.candles||!j.candles.length)return;state.chartData=j.candles;renderChart()}catch(e){}
}
...
function calcEMAvals(vals,period){
  if(vals.length<period)return[];
  const k=2/(period+1);const out=[];
  let ema=vals.slice(0,period).reduce((a,b)=>a+b,0)/period;
  out.push({i:period-1,v:ema});
  for(let i=period;i<vals.length;i++){ema=vals[i]*k+ema*(1-k);out.push({i,v:ema})}
  return out;
}

function renderChart(){
  var box=$('#candles'),paxis=$('#paxis'),vbox=$('#volbars'),tline=$('#timeline');
  Array.from(box.querySelectorAll('.candle,.liveline,.ema-overlay')).forEach(function(el){el.remove()});
  ...
  var maxV=1;
  data.forEach(function(c){if(c.v>maxV)maxV=c.v});
  data.forEach(function(c){
    ...
    el.innerHTML='<div class="wick" style="height:'+(wT-wB)+'%;bottom:'+wB+'%"></div><div class="body" style="height:'+bH+'%;bottom:'+bB+'%"></div>';
    box.appendChild(el);
    if(vbox){
      var vb=document.createElement('div');
      vb.className='volbar '+(c.c>=c.o?'green':'red');
      vb.style.height=Math.max(2,(c.v/maxV)*100)+'%';
      vbox.appendChild(vb);
    }
  });
  var live=livePrices[state.symbol]||data[data.length-1].c;
  var liveB=((live-mn)/range)*100;
  var line=document.createElement('div');
  line.className='liveline';
  line.style.bottom=(8+liveB*(100-8)/100)+'%';
  box.appendChild(line);
  var closes=data.map(function(c){return c.c});
  var e9=calcEMAvals(closes,9),e21=calcEMAvals(closes,21);
  var svg='<svg class="ema-overlay" style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:4" preserveAspectRatio="none" viewBox="0 0 100 100">';
  if(e9.length>1){
    var pts9=e9.map(function(e){var x=(e.i/(data.length-1))*100;var y=100-((e.v-mn)/range)*100;return x.toFixed(2)+','+y.toFixed(2)}).join(' ');
    svg+='<polyline points="'+pts9+'" fill="none" stroke="#f0b90b" stroke-width="0.4" opacity="0.85"/>';
  }
  if(e21.length>1){
    var pts21=e21.map(function(e){var x=(e.i/(data.length-1))*100;var y=100-((e.v-mn)/range)*100;return x.toFixed(2)+','+y.toFixed(2)}).join(' ');
    svg+='<polyline points="'+pts21+'" fill="none" stroke="#1e6cf5" stroke-width="0.4" opacity="0.75"/>';
  }
  svg+='</svg>';
  box.insertAdjacentHTML('beforeend',svg);
  if(tline){
    var now=Date.now();var ms=60000;
    if(state.timeframe==='5m')ms=300000;
    else if(state.timeframe==='15m')ms=900000;
    else if(state.timeframe==='1h')ms=3600000;
    else if(state.timeframe==='4h')ms=14400000;
    else if(state.timeframe==='1d')ms=86400000;
    var labels=[];
    for(var t=4;t>=0;t--){
      var d=new Date(now-t*ms*(data.length/5));
      var hr=d.getHours().toString().padStart(2,'0');
      var mn2=d.getMinutes().toString().padStart(2,'0');
      labels.push(hr+':'+mn2);
    }
    tline.innerHTML=labels.map(function(l){return '<span>'+l+'</span>'}).join('');
  }
}
